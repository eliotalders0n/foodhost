...
function AddProduct() {
  ...
  function displayItems(item) {
    ...
    setSubItem(null);
    setSubItems(item.item);
  }

  const renderItem = ({ item }) => (
    <TouchableOpacity
      onPress={() => displayItems(item)}
      style={{
        paddingVertical: 10,
        marginHorizontal: 5,
        borderRadius: 10,
      ...
      <Modal animationType="slide" transparent={true} visible={modalVisible}>
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            <Text style={{ ...FONTS.h4 }}>
              Thank you, we are adding details to the database
            </Text>
            <Text style={{ textAlign: "center", ...FONTS.h6 }}>
              Please do not leave the page until upload is complete and prompt
              will disappear
            </Text>
          </View>
        </View>
      ...
      >
        ...
        <View style={{ marginTop: 30 }}>
          <Text style={{ ...FONTS.h5, marginVertical: 10 }}>
            Select a category and produce name below
          </Text>
          {doc && (
            <FlatList
              data={doc}
