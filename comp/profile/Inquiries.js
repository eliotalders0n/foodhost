...
function Inquiries() {
  ...
  function updateStatus(id, ProductID, status, quant) {
    console.log(quant);
    ...
    firebase
      ...
      .then(() => {
        firebase
          ...
          .doc(ProductID)
          .update({
            items: firebase.firestore.FieldValue.increment(-quant),
          });
      ...
      {item.status === "pending" ? (
        <View style={{ flexDirection: "row" }}>
          <TouchableOpacity
            ...
            onPress={() =>
              updateStatus(item.id, item.ProductID, "accepted", item.quant)
            }
          >
          ...
          <TouchableOpacity
            ...
            onPress={() => updateStatus(item.id, item.ProductID, "rejected", 0)}
          >
